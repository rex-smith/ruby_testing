...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:wrong_guess) { described_class.new(7, '4')}
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        incorrect_2 = '!'
        valid_input = '4'
        allow(game_loop).to receive(:player_input).and_return(incorrect_1, incorrect_2, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    # @guess, and @count
    subject(:game_two_three) { described_class.new(6, '6', 3)}
    ...
    # Write a test for the following context.
    subject(:game_more_than_four) { described_class.new(8, '8', 7) }
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        hard_phrase = "That was hard. It took you 7 guesses!\n"
        expect { game_more_than_four.final_message }.to output(hard_phrase).to_stdout
      end
    end
  end
end
