...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        incorrect_value_1 = 'peanut'
        incorrect_value_2 = 'corgi'
        valid_input = '6'
        allow(game_input).to receive(:gets).and_return(incorrect_value_1,incorrect_value_2, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        expect(result).to be(valid_input)
      end
    end
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_loop) { described_class.new(1,100) }
    let(:search_loop) { instance_double(BinarySearch)}

    before do
      # You'll need to create a few method stubs.
      allow(search_loop).to receive(:make_guess)
      allow(game_loop).to receive(:display_guess).with(search_loop) # Maybe wrong to have search loop
      allow(search_loop).to receive(:update_range)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do

      expect {game_loop.display_turn_order(search_loop)}.to change { game_loop.instance_variable_get(:@guess_count)}.by(1)
      # Expect increment of 1?
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess to search loop' do
      expect(search_loop).to receive(:make_guess).once
      game_loop.display_turn_order(search_loop)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range to search loop' do
      expect(search_loop).to receive(:update_range).once
      game_loop.display_turn_order(search_loop)
    end
