...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_apple) { described_class.new('apple juice') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_apple.type).to eql('apple juice') 
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_apple.ounces).to eql(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_mate) { described_class.new('mate') }
      # remove the 'x' before running this test
      it 'is full' do
        expect(my_mate).to be_full
      end
    end
